(function (global, factory) {
  ...
  (global = global || self, factory(global.raw = {}));
  ...
  */
  ...
  function getFormatter(dataType, parsingOptions) {
    ...
    if (getType(dataType) === Date) {
      if (isString_1(dataType.dateFormat) && !!dateFormats[dataType.dateFormat]) {
        ...
        var parser = parsingOptions.dateLocale ? formatLocale$1(parsingOptions.dateLocale).parse(mappedFormat) : timeParse(mappedFormat);
        return function (value) {
          if (!value) {
            return EMPTY_DATE_MARKER;
          }
  ...
  function inferTypes(data, parsingOptions) {
    ...
    if (dateLocale) {
      dateParser = formatLocale$1(dateLocale).parse;
    } else {
  ...
  function getPresetScale(scaleType, domain, interpolator) {
    if (scaleType === "sequential") {
      ...
      return sequential(colorPresets.sequential[interpolator].value).domain(domain).unknown(NO_COLOR).clamp(true);
      ...
      return diverging(colorPresets.diverging[interpolator].value).domain(domain).unknown(NO_COLOR).clamp(true);
    } else {
      ...
      return ordinal().domain(finalDomain).range(scaleRange).unknown(NO_COLOR);
    }
  }
  /**
  ...
  function finalizeScale(inputScale, userScaleValuesMapped, scaleType) {
    if (inputScale.range && isEqual_1(inputScale.range().map(function (d) {
      ...
      if (scaleType === "ordinal") {
        ...
        return inputScale.copy().domain(userScaleValuesMapped.domain).interpolator(rgbBasis(userScaleValuesMapped.range));
      }
    }
  }
  ...
  function getDefaultColorScale(defaultColor) {
    return ordinal().unknown(defaultColor);
  }
  /**
      ...
      if (orient === "vertical") {
        (function () {
          ...
          cellTrans = function cellTrans(d, i) {
            var height = sum$1(cellSize.slice(0, i));
  ...
  function size() {
    ...
    function legend(svg) {
      ...
      var maxH = max$2(shapeSize, function (d) {
        return d.height + d.y;
      }),
          maxW = max$2(shapeSize, function (d) {
        return d.width + d.x;
      });
      ...
      if (orient === "vertical") {
        ...
        (function () {
          cellTrans = function cellTrans(d, i) {
            var width = sum$1(shapeSize.slice(0, i), function (d) {
              return d.width;
            });
  ...
  function legend(legendColor, legendSize, legendWidth, shapePadding, shapeWidth, shapeHeight, margin) {
    ...
    var legendFn = function legendFn(_selection) {
      ...
      if (legendSize && legendSize.title) {
        ...
        var _d3LegendSize = size().scale(legendSize.scale).cells(legendSize.scale.domain()).shape(legendSize.shape ? legendSize.shape : "circle").title(legendSize.title).titleWidth(w).labelWrap(w - shapePadding - shapeWidth).labelOffset(5).shapePadding(legendSize.shape === "circle" ? legendSize.scale.range()[1] : shapePadding);
      ...
      if (legendColor && legendColor.title) {
        ...
        d3legendColor = color$1().shapePadding(shapePadding).title(legendColor.title).titleWidth(w).labelWrap(w - shapePadding - shapeWidth).labelOffset(5).scale(legendColor.scale);
  ...
  function labelsOcclusion(d3Selection, priority) {
    ...
    labels.forEach(function (d) {
      select$1(d.node).style("opacity", d._occluded ? 0 : 1);
