...
import { median, group, groups, rollup, rollups, min as min$1, max as max$1, extent, descending } from 'd3-array';
import { timeFormatLocale, timeParse } from 'd3-time-format';
import { color } from 'd3-color';
import { interpolateBlues, interpolateGreens, interpolateReds, interpolateRdBu, interpolateBrBG, interpolatePiYG, schemeCategory10, interpolateTurbo, interpolateSpectral } from 'd3-scale-chromatic';
...
function makeMapper(dimensionsWithOperations, _mapping, types) {
  ...
  return function (data) {
    ...
    if (grouperDimension) {
      ...
      if (groupDimension) {
        return group.apply(void 0, [tabularData].concat(grouperGetters));
      }

      if (groupsDimension) {
        return groups.apply(void 0, [tabularData].concat(grouperGetters));
      }
      ...
      if (rollupGrouperDimension) {
        ...
        var finalRollupFunction = rollupDimension ? rollup : rollups;
...
function getFormatter(dataType, parsingOptions) {
  ...
  if (getType(dataType) === Date) {
    if (isString_1(dataType.dateFormat) && !!dateFormats[dataType.dateFormat]) {
      ...
      var parser = parsingOptions.dateLocale ? timeFormatLocale(parsingOptions.dateLocale).parse(mappedFormat) : timeParse(mappedFormat);
      return function (value) {
        if (!value) {
          return EMPTY_DATE_MARKER;
        }
...
function getPresetScale(scaleType, domain, interpolator) {
  if (scaleType === "sequential") {
    ...
    return scaleSequential(colorPresets.sequential[interpolator].value).domain(domain).unknown(NO_COLOR).clamp(true);
    ...
    return scaleDiverging(colorPresets.diverging[interpolator].value).domain(domain).unknown(NO_COLOR).clamp(true);
  } else {
    ...
    var scaleRange = Array.isArray(interpolatorValue) ? interpolatorValue : quantize(interpolatorValue, domain.length);
...
function getColorDomain(colorDataset, colorDataType, scaleType) {
  ...
  if (sampleDataType === "string" || scaleType === "ordinal") {
    ...
    if (scaleType === "diverging") {
      var minValue = min$1(typedDataset);
      var maxValue = max$1(typedDataset);
      var midValue = 0;
...
function finalizeScale(inputScale, userScaleValuesMapped, scaleType) {
  if (inputScale.range && isEqual_1(inputScale.range().map(function (d) {
    ...
    if (scaleType === "ordinal") {
      ...
      return inputScale.copy().domain(userScaleValuesMapped.domain).interpolator(interpolateRgbBasis(userScaleValuesMapped.range));
    }
  }
}
...
function getInitialScaleValues(domain, scaleType, interpolator) {
  ...
  return domain.map(function (d, i) {
    return {
      domain: d,
      range: color(inputScale(d)).formatHex(),
      index: i
    };
  });
}
...
function overrideBaseOptions(newOptions) {
  baseOptions = _extends({}, baseOptions, {}, newOptions);
}
...
function getOptionsConfig(visualModelOptions) {
  return _extends({}, baseOptions, {}, visualModelOptions || {});
}
/**
...
function legend(legendColor$1, legendSize$1, legendWidth, shapePadding, shapeWidth, shapeHeight, margin) {
  ...
  var legendFn = function legendFn(_selection) {
    ...
    if (legendSize$1 && legendSize$1.title) {
      ...
      var _d3LegendSize = legendSize().scale(legendSize$1.scale).cells(legendSize$1.scale.domain()).shape(legendSize$1.shape ? legendSize$1.shape : "circle").title(legendSize$1.title).titleWidth(w).labelWrap(w - shapePadding - shapeWidth).labelOffset(5).shapePadding(legendSize$1.shape === "circle" ? legendSize$1.scale.range()[1] : shapePadding);
    ...
    if (legendColor$1 && legendColor$1.title) {
      ...
      d3legendColor = legendColor().shapePadding(shapePadding).title(legendColor$1.title).titleWidth(w).labelWrap(w - shapePadding - shapeWidth).labelOffset(5).scale(legendColor$1.scale);
...
export { NumberParser, arrayGetter, baseOptions, chart, colorPresets, dateFormats, deserializeProject$2 as deserializeProject, getAggregator, getAggregatorNames, getAvailableScaleTypes, getColorDomain, getColorScale, getContainerOptions, getDefaultColorScale, getDefaultDimensionAggregation, getDefaultOptionsValues, getDimensionAggregator, getEnabledOptions, getInitialScaleValues, getOptionsConfig, getPresetScale, getTypeName, getValueType, inferTypes, labelsOcclusion, legend, makeMapper, overrideBaseOptions, parseDataset, registerAggregation, registerSerializerDeserializer, serializeProject$2 as serializeProject, translateDateFormat, unregisterAggregation, validateMapperDefinition, validateMapping };
