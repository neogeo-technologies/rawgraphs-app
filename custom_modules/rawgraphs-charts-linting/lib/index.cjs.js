...
var d3Sankey__namespace = /*#__PURE__*/_interopNamespace(d3Sankey);
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const size = d3.scaleSqrt().domain([0, d3.max(data, d => d.size)]).rangeRound([0, maxRadius]);
  ...
  if (mapping.connectedBy.value) {
    ...
    vizLayer.append('path').attr('d', () => line(data.sort((a, b) => {
  ...
  if (showLegend) {
    ...
    if (mapping.size.value) {
      ...
      legendSizeScale.domain(d3.extent(data, d => d.size)).rangeRound([size(d3.min(data, d => d.size)), maxRadius]);
...
function render$1(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'backgorund'); // set up grid
  ...
  series.each(function (d, serieIndex) {
    ...
    const line = d3.line().x(function (d) {
      ...
      return yScale(d.y);
    }).curve(d3__namespace['curve' + interpolation]);

    const xAxis = g => {
      return g.attr('transform', d => 'translate(0,' + (yDomain[0] >= 0 ? serieHeight : yScale(0)) + ')').call(d3.axisBottom(xScale).tickSizeOuter(0)).call(g => g.append('text').attr('x', serieWidth).attr('dy', -5).attr('text-anchor', 'end').attr('display', serieIndex == 0 || repeatAxesLabels ? null : 'none').text(mapping['x'].value).styles(styles.axisLabel));
    };
...
function render$3(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'backgorund');
...
function render$5(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const sankey = d3Sankey.sankey().nodeId(d => d.id).nodeAlign(d3Sankey__namespace[alignments[alignment]]).nodeWidth(nodesWidth).nodePadding(nodesPadding).extent([[0, 0], [chartWidth, chartHeight]]).iterations(iterations);
  const network = sankey({
    nodes,
    links
  }); // add background

  d3.select(svgNode).append('rect').attr('width', width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'backgorund');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz');
...
function render$6(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const xScale = d3.scaleBand().rangeRound([0, chartWidth - nodesWidth]).domain(mapping.steps.value).align(0).paddingInner(1); // update nodes vertical position
...
function render$7(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'backgorund'); // get svg node
...
function render$a(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const padding = (chartHeight - totalValue) / (hierarchy.leaves().length - 1); // dictionary to choose algorythm according to options
...
function render$b(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const padding = (circumference - totalValue) / (hierarchy.leaves().length - 1); // dictionary to choose algorythm according to options
...
function render$c(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, seriesIndex) {
    ...
    if (horizontalBars) {
      ...
      selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(d3.axisBottom(barScale).tickSizeOuter(0)).call(g => g.append('text').attr('x', seriesWidth).attr('y', -4).attr('text-anchor', 'end').attr('display', seriesIndex === 0 || repeatAxesLabels ? null : 'none').text(mapping['bars'].value).styles(styles.axisLabel));
...
const metadata$d = {
  name: 'Diagramme à barres groupées',
  thumbnail: img$p,
  icon: img$o,
...
function render$d(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(d3.axisBottom(setScale).tickSizeOuter(0));
...
const metadata$e = {
  name: 'Diagramme à barres empilées',
...
function render$e(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(d3.axisBottom(stacksScale).tickSizeOuter(0));
    selection.append('g').attr('id', 'yAxis').call(d3.axisLeft(sizeScale).tickSizeOuter(0));
...
function render$h(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  svg.append('g').attr('id', 'x axis').attr('transform', 'translate(0,' + chartHeight + ')').call(d3.axisBottom(xScale)).append('text').attr('x', chartWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['group'].value).styles(styles.axisLabel);
  ...
  }).attr('d', d3.area().x0(d => shapeScale(-d.length)).x1(d => shapeScale(d.length)).y(d => yScale((d.x0 + d.x1) / 2)).curve(curveType[interpolation]));
...
function render$i(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  svg.append('g').attr('id', 'x axis').attr('transform', 'translate(0,' + chartHeight + ')').call(d3.axisBottom(xScale)); //append boxplots
...
function render$l(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    stackedData[0].map((row, rowIndex) => {
      ...
      vStack.sort((a, b) => d3.ascending(a[1], b[1])).sort((a, b) => d3.ascending(a[0], b[0]));
      vStack.forEach((e, i) => {
    ...
    selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + serieHeight + ')').call(d3.axisBottom(xScale).tickSizeOuter(0));
...
function render$n(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    stackedData[0].map((row, rowIndex) => {
      ...
      let vStack = stackedData.map(d => d[rowIndex]);
      let index = 0;
      vStack.sort((a, b) => d3.ascending(a.rankIndex, b.rankIndex)).forEach((e, i) => {
